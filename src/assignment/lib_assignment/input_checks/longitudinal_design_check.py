...
import lib_assignment.assn_utils as assn_utils
from lib_assignment.assn_utils import env_path
import lib_assignment.formatters as formatters
import lib_assignment.input_checks.checker as checker
import lib_assignment.input_checks.exceptions as excp
import lib_assignment.validators as validators
...
def check_longitudinal_design(job):
    ...
    for segment_id in sorted(segments):  # required for deterministic unit test
        ...
        segment = load_json_as_dict(env_path(seg_path, job.vol_base, job.env))
