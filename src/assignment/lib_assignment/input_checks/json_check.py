...
from lib_assignment.input_checks import checker, exceptions
from lib.iotools import load_json_as_dict
...
def name_matches_id(json_file):
    ...
    content = load_json_as_dict(json_file,)
...
def get_jsons(job):
    ...
    Returns:
        ...
        e.g. ((1,2,3), (5,6,7))
    """
    cells = spark.read.csv(env_path(job.config.paths["CELL"], job.vol_base, job.env), header=True).toPandas()
...
def check_jsons(job):
    ...
    for json_type, stack in json_stack.items():
        for identifier in stack["ids"]:
            ...
            path = env_path(job.config.paths[stack["BANK"]], job.vol_base, job.env) + fname
