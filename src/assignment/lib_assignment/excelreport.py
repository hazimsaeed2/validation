...
import lib_assignment.qc as qc
import lib.xlsx_helper as xlh
from lib.iotools import split_path_bucket_key
    ...
    with writer as writer:
        ...
        for table in tables:
            ...
            if isinstance(table, list):
                ...
                write_format_excel_table(
                    terms, table, writer, 1, len(job.data.tables[table].columns) + 4,
                )
...
def write_report_dbx(job, tables, dbutils ):
    """
    Write QC report to Databricks Volume.
    Replaces boto3 upload with local-volume write.
    """

    # 1. ORIGINAL S3 PATH
    s3_path = job.config.paths["QC_REPORT"]

    # 2. MAP TO DATABRICKS VOLUME PATH
    #    Example: /Volumes/catalog/schema/bucket/.../qc_report.xlsx
    output_path = env_path(path=s3_path, vol_base=job.vol_base, env=job.env)


    # 3. CREATE PANDAS EXCEL BUFFER
    buffer = io.BytesIO()
    writer = pd.ExcelWriter(buffer, engine="xlsxwriter")
    ...
    with writer as writer:
        ...
        for table in tables:
            ...
            if isinstance(table, list):
                ...
                terms = pd.DataFrame(definitions)
                # ,
                #     columns=[
                #         "Content",
                #         "Targeting Strategy",
                ...
                fmts = xlh.formats(wb)
                #
                ws.write(0, 0, "Start:", fmts["header1"])
                ws.write(0, 1, start, fmts["header1"])
                ws.write(1, 0, "End:", fmts["header2_bold"])
                ws.write(1, 1, end, fmts["header2_bold"])
                # for col, val in enumerate(["Start:", start]):
                #     ws.write(0, col, val, fmts["header1"])
                # for col, val in enumerate(["End:", end]):
                #     ws.write(1, col, val, fmts["header2_bold"])
