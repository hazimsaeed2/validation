...
def check_prod_status(job):
    ...
    """
    coupon_log = spark.read.csv(env_path(job.config.paths["COUPON_LOG_FOLDER"], job.vol_base, job.env), header=True).toPandas()
    coupon_log.columns = ['campaign','run_name','run_type','log_event','time']
