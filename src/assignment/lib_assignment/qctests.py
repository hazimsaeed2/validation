...
import lib.iotools_assignment as iotools
...
def check_sensitive_content(job):
    ...
    def cleanup(assignment=None, vol_base=None, env='dev'):
        ...
        """
        path = env_path(path=job.config.paths["SENSITIVE_QC"], vol_base=vol_base, env=env)
...
def count_cf_ineligible(job):
    ...
    campaign = Campaign(params, paths, vol_base = job.vol_base, env = job.env)
...
class QCTestRunner:
    ...
    def run_tests(self):
        ...
        for i, test in enumerate(self.torun):
            ...
            print(test_identifier) #self.log.info(
