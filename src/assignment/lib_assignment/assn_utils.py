...
# from lib.iotools import read_s3_to_local
# from lib.spark_util import get_logger
from lib.utils import convert_id_cols_to_str, next_fiscal_week_end
...
def env_path(path, vol_base="/Volumes/datascience_ea_dev/pe/outputs_for_s3", env="dev"):
...
def calc_overlapping_cols(df1, df2):
    ...
    """
    print("Overlapping column calculation on: \n{}\n{}".format(df1, df2))
...
def load_past_longitudinal_mbrs(
    ...
    if len(long_cells) == 0:
        ...
        if mbrs.count() == 0:
            print(f"longitudinal id {longitudinal_id} has no past members")

        check_loaded_long_cells(assignment, long_cells)
