...
from lib_cf.cf_utils import generate_hist
from lib_cf.cf_metrics import (
    hits_at_score,
    median_top_overall,
    median_top_personal,
    lookup_tbl_check,
)
from lib_cf.backtest import (
    prep_backtest,
    run_backtest,
)
...
def purchase_cycle_normalize(params, cat_dna, data, timeperiod):
...
def cat_size_normalize(params, item_dtl, data):
...
def evaluate_cf_train(
    ...
    if backtest:
        print("backtesting...")
        members = sparksession.table('datascience_ea_dev.pe.fs_merge')#sparksession.read.load(paths["CUBE"])
        cat_lookup = sparksession.table('datascience_ea_dev.pe.cf_cat_lookup') # Filter needed dates "s3://memberanalytics-data-out/MODELDATA/DATASETS/cat_lookup_20160101-20180526_AH4_215_v3"
        ...
        bt_pdata = sparksession.table('datascience_ea_dev.pe.cf_matrix') #sparksession.read.parquet(pdata_path)
        bt_fdata = sparksession.table('datascience_ea_dev.pe.cf_matrix') #sparksession.read.parquet(fdata_path)
