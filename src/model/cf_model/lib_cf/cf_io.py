...
def load_config(config_path, test=False, future=False, rmse=False):
    ...
    if not os.path.isfile(config_path):
        msg = "--config: invalid file path: {}".format(config_path)
    ...
    cfg["shared"]["future_run"] = future
    cfg["shared"]["rmse"] = rmse
    return cfg, cfg_path
...
def calculate_filepaths(params, paths):
    ...
    """
    params["p_start"] = params["start"]
    params["p_end"] = params["end"]
