...
def filter_customer_cube(
    ...
    Args:
        ...
        last_date (date): last date registered for cube data
    Returns:
        data (DataFrame)
    """
...
def prepare_data( data, categorical_variables, continuous_variables):
    ...
    # there are at least 3 ways of doing this part. We chose the one that performed the best. Feel free to test the other ones if needed
    #---------------------------------------
    ...
    indexers = [
        StringIndexer(
            inputCols=categorical_variables, outputCols=categorical_variables_out , handleInvalid="keep"
        )
    ]

    # # Option B
    # for col in categorical_variables:
    #     indexer = StringIndexer(inputCol=col, outputCol=col + "_tmp")
    ...
    transformaton_stages = indexers + [assembler_features] # Original and OpA
    #transformaton_stages =  [assembler_features] # OpB
