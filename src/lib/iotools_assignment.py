...
#-------------------------------------------------------------------------------------------------------------------------
# aux methods for move_to_outbound_dbx

def is_volume_file(volume_path, dbutils):
    """
    Check if a Databricks Volume path is a file.
    """

    if not volume_path:
        return False

    # Normalize because ls uses dbfs:/ and user may pass /Volumes/...
    normalized = volume_path.rstrip("/")

    try:
        entries = dbutils.fs.ls(normalized)

        # FILE case: exactly 1 entry AND entry.path == normalized
        if len(entries) == 1:
            entry_path = entries[0].path.rstrip("/")
            if entry_path == normalized or entry_path == f"dbfs:{normalized}":
                return True
...
def list_volume_dir(dir_path, dbutils):
    """
    List all file paths under a Volume directory.
    Returns absolute dbfs:/Volume/... file paths.
    """
