...
def get_archive_path():
    archive = "/archive/stats_{}.yaml".format(
...
def archive_stats(s3_stat_path):
    ...
    """
    archive_path = '/'.join(s3_stat_path.split('/')[:-1]) + get_archive_path()
    shutil.copy(s3_stat_path, archive_path)
...
class DQ_check(object):
    ...
    def get_old_stats(self):
        ...
        """
        try:
            with open(self.s3_stat_path, 'r') as f:
    ...
    def update_old_stats(self, stat_data, encryption="aws:kms"):
        ...
        with open(self.s3_stat_path, "w") as f:
            yaml.dump(stat_data, f, default_flow_style=False, encoding="UTF-8")
...
class TestDuplicates(DQ_check):
    ...
    def test_body(self):
        ...
        """
        self.df.createOrReplaceTempView("df_duplicate_check")
