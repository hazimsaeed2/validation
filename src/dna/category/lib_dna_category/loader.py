...
class Loader:
    ...
    def __load_intermediate(filepath, start, end, spark):
        ...
        df = spark.table(Loader.tables_dict[filepath])
        df = df.filter(df.FISCAL_WEEK_END.between(start, end))
        return df
    ...
    @staticmethod
    def load(spark, reqs, config, tables_dict):

        Loader.spark = spark
        Loader.tables_dict = tables_dict
