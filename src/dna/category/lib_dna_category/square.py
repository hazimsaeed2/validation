...
from lib_dna_category.loader import Loader
...
class Square(object):
    def __init__(self, spark, reqs, config, tables_dict):
        ...
        self.frames = Loader.load(spark, reqs, config, tables_dict)
    ...
    def __square_name(self, path, full_mode=False):
        ...
        """
        self.square = self.square.withColumn('START_DATE', f.lit(self.config["start"]).cast('date'))
        self.square = self.square.withColumn('END_DATE', f.lit(self.config["end"]).cast('date'))
        self.square = self.square.withColumn('RUN_DATE', f.lit(self.__time_string()).cast("timestamp"))
        return path + self.category.lower()
    ...
    def __write(self, path):
        ...
        table = self.tables_dict[square_path]
        # parquet_path = square_path + "/PARQUET/"
        # csv_path = square_path + "/CSV/"
        #self.square.persist(storageLevel=StorageLevel.DISK_ONLY)
        self.spark.sql(f"DELETE FROM {table}")

        fe = FeatureEngineeringClient()
        pk_datatype = self.square.schema[self.category].dataType
        fe.write_table(
            name=table,
            df=self.square.withColumn(self.category, f.coalesce(self.category, f.lit('-1')).cast(pk_datatype)),
            mode="merge"
        )
        # self.square.write.mode('overwrite').saveAsTable()
