...
def feature_atc(job, dna, weeks_back):
    ...
    redemptions together to get total Add-To-Card (ATC)"detail_fiscal"$0job = job_manager.JobManager(spark, intermediate_all_tables_dict, member_dna_config_path)
    redemptions.
    ...
    dna, new_columns = __client_atc_red(dna, job.tables["detail_fiscal"])
    feature_columns.extend(new_columns)

    dna, new_columns = __nat_atc_red(dna, job.tables["payment_fiscal"])
    feature_columns.extend(new_columns)
...
def feature_coupon_clipped(job, dna, weeks_back):
    ...
    dna, features = __num_coupon_clipped(dna, job.tables["coupon_clip_fiscal"])
...
def feature_email_open_rate(job, dna):
    ...
    member_email, features = __get_member_email_L52_weeks(
        dna, job.tables["email"], job.tables["population"]
    )
...
def feature_fiscal_savings_with_clipless(job, dna):
    ...
    surcharge_savings = (
        job.tables["header_fiscal"]
        .alias("header")
        .join(
            job.tables["detail_fiscal"].alias("detail"),
            ["PURCH_HDR_ID"],
            "inner",
        )
    ...
    cpn2_savings = (
        ...
        .alias("payment")
        .join(
            job.tables["header_fiscal"].alias("header"),
    ...
    gas_savings = (
        ...
        .alias("detail")
        .join(
            job.tables["member_history"].alias("mem_hist"),
            ["MBRSHP_SID", "EFF_DT"],
            "inner",
        )
...
def feature_cpn_channel(job, dna):
    ...
    detail = job.tables["detail_fiscal"]
    ...
    detail = detail.withColumn(
        "frac_paper",
        sqlf.coalesce(sqlf.try_divide(sqlf.size(sqlf.collect_set("paper_cpn").over(window))
        , sqlf.col("cpns"))
        , sqlf.lit(0))
    )
    detail = detail.withColumn(
        "frac_digital",
        sqlf.coalesce(sqlf.try_divide(sqlf.size(sqlf.collect_set("digital_cpn").over(window))
        , sqlf.col("cpns"))
        , sqlf.lit(0))
    )
