...
def feature_stdev(job, dna):
    ...
    skeleton = job.tables["population"]
...
def feature_distinct_categories(job, dna):
    ...
    ah4s = job.tables["detail_isnr_fiscal"].select(*sel).dropDuplicates()

    ah4s = ah4s.withColumnRenamed("MBRSHP_SID", "_sid").withColumnRenamed(
        "FISCAL_WEEK_END", "_fwe"
    )

    feature_population = job.tables["feature_population"]
...
def feature_trip_intervals(job, dna, weeks):
    ...
    trips = __trip_days(job.tables["detail_isnr_fiscal"])
...
def __all_preprocessing(job, dna):
    ...
    Returns:
        (pyspark.sql.DataFrame): dna with new features
    """
    agg_payment = __preprocess_payment(job.tables["payment_fiscal"])

    tender_map = __preprocess_tender_map(job.tables["tender_map"])
...
def __loop_through_groups(job, dna, gen, tender_group_spend, lb_weeks):
    ...
    """
    final_output = job.tables["feature_population"]
    ...
    for gtt in gen:
        ...
        lb_df, new_columns = __do_lookback(
            job.tables["population"], filtered, gtt, window, lb_weeks
        )
...
def __aggregate_daily(job, dna, metric_column):
    ...
    rel_det = job.tables["detail_isnr_fiscal"].select(
        "MBRSHP_SID",
        "FISCAL_WEEK_START",
        "FISCAL_WEEK_END",
        "PURCH_DT",
    ...
    rel_det = rel_det.union(
        job.tables["detail_fiscal"].select(
            "MBRSHP_SID",
            "FISCAL_WEEK_START",
            "FISCAL_WEEK_END",
            "PURCH_DT",
