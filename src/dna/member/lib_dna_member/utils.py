...
def window_dates(job):
    ...
    """
    cube_config = job.config["params"] or {}
    if not ({"start_date", "end_date"} - cube_config.keys()):
        ...
        job.config["params"]["start_date"] = start_date
        job.config["params"]["end_date"] = end_date
    else:
        raise Exception(
...
def next_fiscal_week_end(job):
    ...
    return datetime.datetime.strftime(first_fiscal_week_end, "%Y-%m-%d")
