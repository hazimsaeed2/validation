...
def feature_preferred_club(job, dna):
    ...
    population = job.data.tables["population"]
...
def feature_strategic_segment(job, dna):
    ...
    census_segment = __get_segment(census, job.data.tables["segment"])
...
def feature_preferred_club_has_gas(job, dna, weeks):
   ...
   Returns:
    ...
    fwe_skeleton = (
        job.data.tables["feature_population"]
        .select("FISCAL_WEEK_END")
        .distinct()
    )

    club_has_gas = __club_has_gas(
        job.data.tables["club"], fwe_skeleton, preferred_club_nbr
    )
...
def __percent_trips_l(dna):
    ...
    Returns:
        (pyspark.sql.DataFrame, list(str)): dataframe with the new column,
            new columns
    """
    percent = dna.L52W_PREFERRED_CLUB_TRIPS / dna["LAST_FIFTY-TWO_WEEK_TRIPS"]
...
def __calc_last_over_prior(df, lag, col, feature_name):
    ...
    return df.withColumn(
        feature_name, col.cast("double") / sqlf.lag(col, offset=lag).over(w)
    )
