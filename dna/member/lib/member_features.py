...
def feature_member_master(job, dna):
    ...
    """
    join_col = ['MBRSHP_SID']
    mod_member = __latest_rename(job.data.tables["member"], join_col)
...
def feature_member_extended(job, dna):
    ...
    join_col = ['MBRSHP_SID']
    mod_member_ext = job.data.tables["member_extended"].select(
        *select_cols
    )
...
def feature_trial_member_ind(job, dna):
    ...
    mbrshp_fee_inc_zero = sqlf.when(
        job.data.tables["member_extended"].MBRSHP_FEE_INC == 0, 1
    ).otherwise(0)

    trial_mem_condition = sqlf.when(
        job.data.tables["member_extended"].MKT_CD.like('Z%'),
        mbrshp_fee_inc_zero
    ).otherwise(0)

    trial_mem = job.data.tables["member_extended"].withColumn(
...
def feature_member_history(job, dna):
    ...
    dna = dna.join(
        job.data.tables["member_history"].drop('FISCAL_WEEK_END'),
        ['MBRSHP_SID', 'EFF_DT'],
        'left_outer'
    )
...
def feature_calculated_mfi(job, dna):
    ...
    Returns:
        (pyspark.sql.DataFrame): dna with new features
    """
    member_history = job.data.tables["member_history"].select(
...
def feature_distance(job, dna):
    ...
    census_tract = job.data.tables["census_tract"].drop(*drop_cols)
...
def __partial_aggregation(job, dna):
    ...
    mem_hist_keys = job.data.tables["member_history"].select(
