...
from pyspark.sql.functions import when, row_number, desc, countDistinct, first, col, lit
...
# RAW input schemas
BRAND_SCHEMA = StructType([StructField("ARTICLE_NBR", LongType(), True),
                           StructField("CASE_EXPRESSION", StringType(), True),
                           StructField("BRAND", StringType(), True)])

# output schemas
EXCLUSION_SCHEMA = StructType([StructField("CATEGORY_TYPE", StringType(), False),
...
def preprocess_transactions(trans):
    ...
    fy17 = (col("FISCAL_WEEK_END") > lit("2017")) & (col("FISCAL_WEEK_END") < lit("2018"))
    valid_trans = in_store & non_return & fy17
    in_store_year = trans[valid_trans]
    by_art = in_store_year.groupBy('ARTICLE_NBR').agg(fsum('EXTENDED_PRC_AMT').alias('FY18_SALES'))
    return by_art
...
def get_top_ah5_cat(branddata):
    ...
    # group and count
    sales_by_brand = brand.groupBy('BRAND').agg(fsum('FY18_SALES').alias('total_sales'))
    by_lvl = brand.groupBy('BRAND', group_col, incl_col).agg(countDistinct('ARTICLE_NBR').alias('count'),
    ...
    by_lvl = by_lvl.withColumn('weighted_count', col('count') * col('pct_sales'))

    # window to get top brand
    w = Window.partitionBy('BRAND')
    by_lvl = by_lvl.withColumn("rn", row_number().over(w.orderBy(desc("weighted_count"))))
...
def calc_stats_by_brand(branddata, cat_dna):
    ...
    """
    # group data
    by_brand = branddata.groupBy('BRAND').agg(countDistinct('ARTICLE_NBR').alias('N_ART'),
    ...
    by_brand = by_brand.withColumn("PCT_TOP_COVG", by_brand.TOP_CAT_ART_CT / by_brand.N_ART)
    ...
    included_pct = (col('include') / col('total'))
    by_brand = by_brand.withColumn('INCL_COVG', when(is_excluded, excluded_pct).otherwise(included_pct))
...
def include_brands(branddata, included_brands):
    ...
    if "BRAND" not in included_brands.columns:
        included_brands = included_brands.withColumnRenamed('CATEGORY_DESCRIPTION', 'BRAND')
...
def exclude_brands(branddata, exclusions, excluded_brands, reshape=True):
    ...
    if "BRAND" not in excluded_brands.columns:
        excluded_brands = excluded_brands.withColumn('BRAND', excluded_brands.CATEGORY_DESCRIPTION)
    ...
    if reshape:
        ...
        base_cols = ['BRAND', 'TOP_CAT_INCL', 'EXCLUSION_TYPE', 'EXCLUSION_SUBTYPE']
        select_cols = base_cols + snl_cols
        reshaped_excludes = excluded_brands.select(select_cols)

        reshaped_excludes = reshaped_excludes.withColumn('CATEGORY_TYPE', lit('brand'))
        reshaped_excludes = reshaped_excludes.withColumn('CATEGORY_DESCRIPTION', reshaped_excludes.BRAND)
        reshaped_excludes = reshaped_excludes.withColumnRenamed('BRAND', 'CATEGORY_CD')

        # we use the top category as template for exclusion type and reasoning
        reshaped_excludes = reshaped_excludes.withColumnRenamed('TOP_CAT_INCL', 'INCLUDE_OR_EXCLUDE')
        final_col_order = ['CATEGORY_TYPE', 'CATEGORY_CD', 'CATEGORY_DESCRIPTION',
                           'INCLUDE_OR_EXCLUDE', 'EXCLUSION_TYPE', 'EXCLUSION_SUBTYPE']
...
if __name__ == "__main__":
    ...
    base_cols = ['AH5_CD', 'AH5_DESC', 'INCLUDE_OR_EXCLUDE', 'EXCLUSION_TYPE', 'EXCLUSION_SUBTYPE']
    selection_cols = base_cols + snl_cols
    cat_dna = cat_dna.select(*selection_cols)

    transactions = spark.read.parquet(paths['transactions'])
    transactions = transactions.select('ARTICLE_NBR', 'SALES_CTGRY_CD',
                                       'FISCAL_WEEK_END',  'EXTENDED_PRC_AMT')

    manually_mapped = spark.read.csv(paths['exclusions'], header='true', schema=EXCLUSION_SCHEMA)
    ...
    by_brand, exclusions = exclude_brands(by_brand, exclusions, single_rule_excludes)
    by_brand.cache()
    exclusions.cache()
    ...
    by_brand, exclusions = exclude_brands(by_brand, exclusions, pure_mixed_excludes)
    by_brand.cache()
    exclusions.cache()
    ...
    by_brand, exclusions = exclude_brands(by_brand, exclusions, manual_excludes, reshape=False)
    by_brand.cache()
    exclusions.cache()
