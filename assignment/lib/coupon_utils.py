...
def append_non_duplicates(spark, existing, df, path, ftype, dedupe_col=None):
    ...
    if ftype.lower() == "parquet":
        deduped.coalesce(1).write.parquet(path, mode="overwrite")
    else:
        deduped.coalesce(1).write.csv(path, mode="overwrite", header=True)
    deduped.unpersist()


def append_new_campaign(job, new, path_id, ftype, cpn_quals, del_keys):
    ...
    if ftype.lower() == "parquet":
        existing.coalesce(1).write.parquet(
            job.config.paths["{}_TEMP".format(path_id)], mode="overwrite"
        )
    else:
        existing.coalesce(1).write.csv(
            job.config.paths["{}_TEMP".format(path_id)],
            mode="overwrite",
            header=True,
        )
    ...
    append_non_duplicates(
        job.spark, modified, new, job.config.paths[path_id], ftype, "cpn_nbr"
    )
...
def cast(df, cols=None):
    ...
    for column in cols:
        if column in [
            ...
            _debug_column_sample(df, column, "before cast(long)")
            df = df.withColumn(column, col(column).cast("long"))
            _debug_column_sample(df, column, "after cast(long)")
...
def format_coupons(
    ...
    if couptype.lower() == "article":
        ...
        substi_excl = []
    log.info("Exclude coupons for backfilling{}...".format(cpn_excl))
    cpn_incl = [
