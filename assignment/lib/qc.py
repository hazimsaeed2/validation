...
def generate_coupon_dataset(job):
    ...
    Returns:
        coupon (pyspark.sql.DataFrame): "coupon" dataset
            ...
            IMPRESSIONS (int)
    """
    job.log.info("Generating coupon impressions")
...
def generate_compare_dataset(job, key):
    ...
    """
    job.log.info("Generating cell summary by {}".format(key))
    ...
    for selector in selectors:
        basedata = basedata.withColumn(
            selector,
            sqlf.when(sqlf.col(selector).isNull(), sqlf.lit("NULL")).otherwise(
                sqlf.col(selector)
            ),
        )
...
def generate_decile_by_mail_dataset(job):
    ...
    """
    job.log.info("Generating decile mail split")
...
def generate_renewal_dataset(job, key):
    ...
    """
    job.log.info("Generating renewal summary by {}".format(key))
    key = key.upper()
...
def generate_personalized_dataset(job, key):
    ...
    """
    job.log.info("Generating personalized content by {}".format(key))
    key = key.upper()
    base = job.data.tables["base"].dropDuplicates(["MBRSHP_SID", key])
    if key == "CPN_NBR":
...
def generate_descriptive_dataset(job, key):
    ...
    """
    job.log.info("Generating descriptive summary by {}".format(key))
    key = key.upper()
    dna = job.data.tables["dna"]
    base = job.data.tables["base"].dropDuplicates(["MBRSHP_SID", key])
    if key == "CPN_NBR":
...
def generate_savings_dump(job):
    ...
    """
    job.log.info("Creating output file for savings content.")
...
def generate_savings_overview(job):
    ...
    """
    job.log.info("Generating savings overview tables")
...
def generate_savings_keyed(job, key):
    ...
    """
    job.log.info("Generating savings summary by {}".format(key))
    base = job.data.tables["base"]
    if key == "CPN_NBR":
...
def generate_nomail_dataset(job):
    ...
    """
    job.log.info("Generating no mail dataset")
...
def generate_example_data(job, examples):
    ...
    """
    job.log.info("Generating Examples")
    basedata = job.data.tables["base"]
    cpn_articles = job.data.tables["cpn_articles"]
    ...
    for example in full_examples:
        job.log.info("Generating example {}".format(example))
        table = basedata[basedata["MBRSHP_SID"] == example["id"]].join(
...
def generate_coupon_articles(job):
    ...
    :return:
    """
    job.log.info("Generating coupon articles")
    ...
    category_coups = (
        ...
        .drop("ARTICLE_NBR")
        .join(ah5.union(ah4), "CATEGORY_ID")
        .select(cols)
        .distinct()
    )
...
def generate_configured_qc(job):
    ...
    :return:
    """
    job.log.info("Generating configured qc")
    base = job.data.tables["base"]
    ...
    for extra_info in [
        ...
        job.log.info("Adding {} to the qc.".format(extra_info))
        base = base.join(
    ...
    if job.config.params["aggregation_params"]:
        ...
        for dummy in aggregation_params:
            ...
            for i in range(0, len(aggregation_param["group_by"])):
                ...
                job.log.info("Grouping by {}.".format(group_by))
                dict_aggs = {}
...
def pct_trial(job, key=None):
    ...
    def has_trial(job):
        ...
        if "CellName" not in mail.columns:
            job.log.warn("Trial flag 'CellName' not present in mail list.")
            return False
        ...
        if len(present.intersection(expected)) != 2:
            job.log.warn(
                "Trial values not found matching standard: {}. Only trial related values found were: {}".format(
                    expected, present
                )
            )
            return False
...
def generate_full_basedata(job, TYPE):
    ...
    """
    job.log.info("Generating base dataset")
    experiment_id = job.config.params["experiment"]
    assignment = (
    ...
    if TYPE == 1 and job.config.paths.get("VERSION_MAP") is not None:
        job.log.info("Reading version map for BBM.")
    ...
    job.log.info("Assignment Count: {}".format(count_check))
    basedata = assignment.join(couptypes, "CPN_NBR", "left")
    job.log.info("Post Coupon Join: {}".format(basedata.count()))
    basedata = basedata.join(
        constructs, ["MBRSHP_SID", "CPN_NBR", "CONSTRUCT"], "left"
    )
    job.log.info("Post Construct Join: {}".format(basedata.count()))
    basedata = basedata.join(trips, ["MBRSHP_SID", "CPN_NBR"], "left")
    job.log.info("Post Member Trips join: {}".format(basedata.count()))
    basedata = basedata.join(dna, ["MBRSHP_SID"], "left").persist(
        StorageLevel.MEMORY_ONLY
    )
    job.log.info("Post DNA join: {}".format(basedata.count()))
...
def generate_member_dataset(job):
    ...
    """
    job.log.info("Generating member level trips and spend")
    basedata = job.data.tables["base"]
...
def generate_longitudinal_dataset(job):
    ...
    """
    job.log.info("Generating longitudinal qc dataset")
