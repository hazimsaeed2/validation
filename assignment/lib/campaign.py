...
from pe_member_dna.assignment.lib.filters import filter_functions
from pe_member_dna.assignment.lib.ingest import (
    broadcast_filter,
    join_category_agnostic,
)
import pe_memberdna.assignment.lib.schemas.cdsa_schemas as cdsa_schemas
from pe_member_dna.assignment.lib.slots import (
    fill_slot,
    offer_data_to_list,
)
from pe_member_dna.lib.iotools import (
    read_s3_to_local,
    split_path_bucket_key,
    write_json_to_s3,
)
from pe_member_dna.lib.spark_util import (
    get_logger,
    truncate_history,
    union_with_mismatched_columns,
)
from pe_member_dna.lib.utils import (
    apply_unionall,
    stack,
...
def _run_slot(
    ...
    log.info("Calculating offer data for slot group.")
    if backfill:
    ...
    if isinstance(slot_group.get("exclude_exp_channel"), dict):
        log.info("need to avoid back to back for specific offer data: ")
        ...
        log.info("need to avoid back to back for all offer data: ")
        slot_subset_data = dict()
        for offer_type in slot_offer_data_type:
            slot_subset_data[offer_type] = _exclude_exp(
                assignment_pools[offer_type],
    ...
    log.info("Finished calculating offer for slot group: %s" % slot_group)

    return assignments


def _read_and_subset_exp_csv(experiment_name, experiment_path, columns=None):
    ...
    """
    df = read_s3_to_local(experiment_path, ftype="csv", columns=columns)
    rel_df = df[df.experiment_id == experiment_name]
    return rel_df


def _parse_campaign_details(experiment_name, experiment_path):
    ...
    """
    rel_dtls = _read_and_subset_exp_csv(experiment_name, experiment_path)
...
def _build_construct_structure(construct, paths):
    ...
    for construct_id in construct_ids:
        ...
        log.info("parsing construct {}".format(construct_id))
        single_json_data = read_s3_to_local(fpath, ftype="json")
    ...
    if len(construct_ids) > 1:
        ...
        write_json_to_s3(bucket, key, construct_structure)
    else:
    ...
    if not is_default_mapping and construct_structure.get("sort"):
        log.warn(
            "Construct {construct} has both mapping and sort"
...
def _parse_constructs(cells, paths, backfill_default_constructs):
    ...
    all_constructs = read_s3_to_local(paths["CONSTRUCT"], ftype="csv")
...
def _build_segment_structure(segment, paths):
    """Builds a dict object composed of all information read from all ids found
    in segment.
    ...
    for segment_id in segment_ids:
        ...
        log.info("parsing segment {}".format(segment_id))
        single_json_data = read_s3_to_local(fpath, ftype="json")
    ...
    if len(segment_ids) > 1:
        ...
        bucket, key = split_path_bucket_key(write_path)
        write_json_to_s3(bucket, key, segment_to_save)
    else:
...
def _parse_segments(cells, paths):
    """Parse segment data.
    ...
    all_segments = read_s3_to_local(paths["SEGMENT"], ftype="csv")
    ...
    for segment in unique_segments:
        if segment and segment not in segments:
            segments[segment] = _build_segment_structure(segment, paths)
            log.info("added segment {} to segment_list".format(segment))
...
class Campaign:
    ...
    def __init__(self, parameters, paths):
        """Initialize parser.
        ...
        (
            ...
            self.channel,
            self.size,
        ) = _parse_campaign_details(self.experiment, self.paths["CAMPAIGN"])
        ...
        self.offer_sources, self.slots, self.lambdas = _parse_constructs(
            self.cells, self.paths, self.backfill_default_constructs
        )
    ...
    def ingest_member_data(self):
        ...
        for source in self.member_sources:
            log.info("ingesting {} to member data...".format(source["path"]))
    ...
    def ingest_offer_data(self):
        ...
        article_map = read_subset_and_cast(
            self.paths["ARTICLE_AH4_AH5_MAP"], "parquet", col_names
        )

        for names in col_names:
            article_map = article_map.withColumnRenamed(names, names.lower())

        trips = read_subset_and_cast(self.paths["COUPON_MEMTRIP"], "parquet")
        trips = trips.dropna(subset=["cpn_nbr", "mbrshp_sid"])
        usage = read_subset_and_cast(self.paths["COUPON_MEMUSAGE"], "parquet")
        cf_pred = read_subset_and_cast(self.paths["PRED_LIST"], "parquet")
        ...
        log.info("Repartitioning and caching cf predictions and trips.")
        trips.count()
        cf_pred.count()
        log.info("Done")
        ...
        for offer_data_type in list(set(self.offer_list)):
            ...
            if self.parameters["downsample_coupons"]:
                log.info("Downsampling offer data:")
    ...
    def calculate_exposure(self):
        ...
        log.info("generate mbr offer recency: ")
        ...
        log.info("generate mbr cpn type recency: ")
    ...
    def run_filters(self, memberdata):
        ...
        for filt in self.filters:
            log.info("filter for construct by {}".format(filt["filter_type"]))
    ...
    def assign_cells(self, memberdata):
        ...
        checks.check_multi_long_tests(memberdata, cells)
    ...
    def pivot_assignment(self, assignment):
        ...
        log.info("Pivoting assigned offers.")
