...
def calculate_filepaths(params, paths):
    ...
    if params["run_size"].lower() == "full":
        ...
        log.warn("Running with sample size of: {}".format(params["run_size"]))
    ...
    for key, path in paths.items():
        if path is None and key in optional_file_paths:
            ...
            if isinstance(path, str):
                paths[key] = re.sub("(?<!s3:)\/\/", "/", path)
            else:
                for i, p in enumerate(path):
                    paths[key][i] = re.sub("(?<!s3:)\/\/", "/", p)
...
class DataManager:
    ...
    def _parse_schemas(self):
        ...
        """
        from pe_member_dna.pipelines.assignment.lib.schemas.assn_schemas import (
            ASSIGNMENTS,
        )
    ...
    def read(
        ...
        if not required and pathname not in list(self.paths.keys()):
            ...
            if (
                ...
                path = generate_outbound_path(
                    name, filetype, self.params, self.paths
                )
            else:
            ...
            if readtype == "distributed":
                ...
                table = read_s3_to_local(path, ftype=filetype)
            else:
