...
from pe_member_dna.pipelines.assignment.lib.input_checks import checker
...
def input_coupon_check_wrapper(
    ...
    validators = [
        ...
        (
            "column_empty",
            check_column_empty(df, column_check_empty, ignore_empty_rows),
        ),
    ...
    if column_check_date_format:
        validators.append(
            (
                "coupon_date_format",
                check_date_format(df, column_check_date_format),
            )
        )
    if earlier_date and later_date:
        validators.append(
            (
                "coupon_date_range",
                check_date_range(df, earlier_date, later_date),
            )
        )
...
def check_coupons(job):
    ...
    job.data.read(
        "article", "CPG_COUPON_LIST_PATH", filetype="csv", required=False
    )
    job.data.read(
        "category", "CATEGORY_COUPON_PATH", filetype="csv", required=False
    )
    job.data.read(
        "basket", "BASKET_COUPON_PATH", filetype="csv", required=False
    )
    job.data.read(
        "prices", "COUPON_PRICE_PATH", filetype="csv", required=False
    )
    job.data.read(
        "closure_coupon", "CLOSURE_COUPON_PATH", filetype="csv", required=False
    )

    # TODO: Can this be converted to job.data.read?
    if job.config.paths.get("EXTRA_INFO_CLOSURE_COUPON_LOOKUP_PATH"):
        closure_lkup = read_subset_and_cast(
            job.config.paths["EXTRA_INFO_CLOSURE_COUPON_LOOKUP_PATH"], "csv"
        )
    ...
    if job.data.tables.get("article"):
        job.log.info("-----Checking article.csv: ")
    ...
    if job.data.tables.get("category"):
        job.log.info("-----Checking category.csv: ")
    ...
    if job.data.tables.get("basket"):
        job.log.info("-----Checking basket.csv: ")
        checks.extend(
            input_coupon_check_wrapper(
    ...
    if job.data.tables.get("prices"):
        ...
        job.log.info("-----Checking {}: ".format(file))
    ...
    job.log.info("coupon_input_check.py: All coupon checks passed")
    return checks
