...
def convert_to_spark_dataframe(spark, date_list):
    schema = StructType([StructField('FISCAL_DAY', DateType(), True),
    ...
    return spark.createDataFrame(date_list, schema=schema)
...
def main(spark, data_paths, config_validation):
    ...
    date_format = '%Y-%m-%d'
    start_date = dt.datetime.strptime(data_paths['fiscal_days']['start'], date_format)
    end_date = dt.datetime.strptime(data_paths['fiscal_days']['end'], date_format)

    out = data_paths['intermediate']['fiscal_days']

    date_list = iterate_fiscal_weeks(start_date, end_date)
    fiscal_days = convert_to_spark_dataframe(spark, date_list)
