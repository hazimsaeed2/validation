...
def main(spark, data_paths, config_validation):
    ...
    logging.info('Reading the input file ' + data_paths['source']['member_history'])

    df = spark.read.csv(data_paths['source']['member_history'],
    ...
    micro_cases = df.filter((df['count'] > 1) & (df.CLUB_OF_FREQUENCY.isNotNull()))
    df = df.filter(df['count'] < 2)
    df = df.union(micro_cases)

    fiscal_days = spark.read.parquet(data_paths['intermediate']['fiscal_days'])
    df = df.join(fiscal_days.select('FISCAL_DAY', 'FISCAL_WEEK_END'), df.EFF_DT == fiscal_days.FISCAL_DAY, 'left_outer').drop('FISCAL_DAY', 'DIFF', 'count')
